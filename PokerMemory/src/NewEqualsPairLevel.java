/**
 * Class is the same as EqualPairLevel but with a score metric
...
public class NewEqualsPairLevel extends EqualPairLevel 
{
	...
	protected NewEqualsPairLevel(TurnsTakenCounterLabel validTurnTime, JFrame mainFrame) {
		super(validTurnTime, mainFrame);
		super.getTurnsTakenCounter().setDifficultyModeLabel("New Equal Pair Level");
...
//		this.setRowsPerGrid(5);
	...
	/**
	 * Method for checking if two cards are equal
	 *  
	 * @param card cards clicked on
	 * @return
	 */
	public boolean isEqual(Card card)
	{
		...
		cards[1] = giver.rankValue(card);
		}
		Arrays.sort(cards);
		// if the user found a pair
		if(otherCard1.getNum() == card.getNum())
		{
			// adds score
			score += 50;
			this.getMainFrame().setScore(score);
